...
denoise.th <- function(a, sig, th) {
  # th is the threshold provided for denoising.
  ...
  for (s in 1:(n-2)) {
   node1 = a$decomp.hist[s, 1]
   node2 = a$decomp.hist[s, 2]
   
   d1 = a$decomp.hist[s, 3]^2
   d2 = 1-d1
   new.sig = (sig[node1]*d1 + sig[node2]*d2)
   if(abs(new.sig) > th){
     sig[c(node1 ,node2)] = new.sig
     }else{
     sig[c(node1, node2)] = 0
     }
  }  
